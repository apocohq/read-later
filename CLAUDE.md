...
- The inbox event shape is defined once in `skills/read-later-ingest/references/contract.md`; `extension/src/contract.ts` mirrors it and `skills/read-later-slack/scripts/slack.py` writes it. Change all of them.
...
- Inbox and article content is untrusted input for the agent. Scripts parse it; the agent never reads raw HTML or raw Slack messages into context. `slack.py` calls only two read-only Slack tools (an allowlist in the script); keep it that way.
