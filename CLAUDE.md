...
- `skills/` — Agent Skills (agentskills.io spec) installed onto DAM agents from this repo, one capability each, prefixed `read-later-`. Today only `read-later-ingest` (inbox events → clean Markdown articles); evaluate, slack-sweep and deliver follow the same pattern. Dependencies are declared inline (PEP 723) and resolved by `uv run` on first use; nothing is bundled.
