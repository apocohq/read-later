...
Chrome and Arc, Manifest V3. One click captures the current page (URL, title, selection, and the rendered HTML with scripts and styles stripped; for a PDF tab just URL and title, the agent fetches the file) and uploads it as a file into your agent's workspace on DAM. The agent's next refresh picks it up from `work/read-later/inbox/`.
